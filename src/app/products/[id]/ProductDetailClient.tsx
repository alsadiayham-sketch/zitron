    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-[var(--primary)] mb-4">المنتج غير موجود</h1>
      ...
      <div className="bg-gray-50 py-3">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span>/</span>
            <Link href="/products" className="hover:text-[var(--primary)]">المنتجات</Link>
            <span>/</span>
      ...
      <section className="py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12">
            {/* Image */}
            <div className="bg-[var(--secondary)] rounded-2xl p-8 flex items-center justify-center relative">
              {product.badge && (
                <span className="absolute top-4 right-4 bg-[var(--accent)] text-white text-sm px-4 py-1 rounded-full">
                  {product.badge}
                </span>
              )}
            ...
            {/* Details */}
            <div>
              <p className="text-[var(--primary)] font-medium text-sm mb-2">{product.range}</p>
                ...
                <div className="flex gap-0.5">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Star
                      key={i}
                      size={16}
                      className={i <= product.rating ? "fill-[var(--star)] text-[var(--star)]" : "text-gray-300"}
                    />
                  ))}
                </div>
              ...
              <div className="flex flex-wrap gap-4 mb-6 text-sm">
                ...
                {product.skinType.map((st) => (
                  <span key={st} className="bg-[var(--primary)]/10 text-[var(--primary)] px-3 py-1 rounded-full">
                    {st}
                  </span>
                ))}
              </div>
              ...
              {/* Benefits */}
              <div className="mb-8">
                <h3 className="font-bold text-sm mb-3">الفوائد الرئيسية:</h3>
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-3 px-8 rounded-full font-medium transition-all ${
                    added
              ...
              {/* Free shipping note */}
              {product.price >= 49 && (
                <p className="text-green-600 text-sm mt-4">✓ هذا المنتج مؤهل للشحن المجاني</p>
              )}
            </div>
          </div>
